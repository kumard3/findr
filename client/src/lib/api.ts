...
export class SearchAPI {
  ...
  private async validateApiKey(): Promise<{ userId: string }> {
    try {
      const response = await fetch(`${SEARCH_API_URL}/api/validate-key`, {
      ...
      if (!response.ok) {
        ...
        throw new Error(error.error || "Invalid or expired API key");
      }

      const data = await response.json();
      this.userId = data.userId;
      this.collectionName = `collection_${this.userId}`;
      return data;
    } catch (error) {
  ...
  async generateApiKey(data: z.infer<typeof ApiKeySchema>) {
    try {
      const response = await fetch(`${SEARCH_API_URL}/api/keys`, {
        method: "POST",
  ...
  async getUsage(): Promise<
    ApiResponse<{ used: number; limit: number; storage: number }>
  > {
    try {
      const response = await fetch(`${SEARCH_API_URL}/api/usage`, {
        headers: this.getHeaders(),
      });
